...
module ``about the stock example`` =
    ...
    let stockRecords = stockData |> List.skip 1 |> List.map splitCommas

    let variability(record : string[]) =
        let openPrice = System.Double.Parse(record.[1], CultureInfo.InvariantCulture)
        let closePrice = System.Double.Parse(record.[4], CultureInfo.InvariantCulture)

        let difference = openPrice - closePrice

        abs difference

    let mostVariable =
        stockRecords
        |> List.sortByDescending variability
        |> List.head

    [<Koan>]
    let YouGotTheAnswerCorrect() =
        let result = mostVariable.[0]
